...
<?php else: ?>
  ...
  <ul <?php if ($wrapper_classes) { print 'class="' . $wrapper_classes . '" '; } ?>> 
  <?php foreach($data as $row_count => $row): ?>
    <li <?php if ($row_classes[$row_count]) { print 'class="' . $row_classes[$row_count] .'"';  } ?>>
    <?php foreach($row as $field_name => $content): ?>
      <div <?php if ($field_classes[$field_name]) { print 'class="' . $field_classes[$field_name] . '" '; } ?>>
        <?php print $content ?>
      </div>
    <?php endforeach ?>
    </li>
